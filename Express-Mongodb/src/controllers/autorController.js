import {autor} from "../models/Autor.js";

class AutorController{
    static async listarAutores(req,res){
        try{
            const listaAutor = await autor.find({})
            res.status(200).json(listaAutor)
        }catch(erro){
            res.status(500).json({ mesage: `${erro.mesage} - Falha na requisição`})
        }
    
    }
    static async listarAutorPorId(req,res){
        try{
            const id = req.params.id
            const autorEncontrado = await autor.findById(id)
            res.status(200).json(autorEncontrado)
        }catch(erro){
            res.status(500).json({ mesage: `${erro.mesage} - Falha na requisição do Autor`})
        }
    }
    
    static async cadastrarAutor(req,res){
        try{
            const novoAutor = await autor.create(req.body)
            res.status(201).json({
                mesage:'Criado com sucesso',
                autor: novoAutor
            })
        }catch(erro){
            res.status(500).json ({mesage: `${erro.mesage} - Falha ao tentar cadastrar novo Autor`})
            console.log(erro)
        }
    }
    
    static async atualizarAutor(req,res){
        try{
            const id = req.params.id
            await autor.findByIdAndUpdate(id, req.body)
            res.status(200).json({message: "Autor atualizado"})
        }catch(erro){
            res.status(500).json({ mesage: `${erro.mesage} - Nao foi possivel atualizar o autor.`})
        }
    }
    static async excluirAutor(req,res){
        try{
            const id = req.params.id
            await autor.findByIdAndDelete(id)
            res.status(200).json({message: "Autor excluido com sucesso."})
        }catch(erro){
            res.status(500).json({ mesage: `${erro.mesage} - Nao foi possivel atualizar o exclusão.`})
        }
    }
}

export default AutorController;